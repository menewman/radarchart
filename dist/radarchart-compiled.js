...
window.radarchart = function () {
  function createRadarChartFactory(canvasSize, maxRadius, numAxes) {
    if (numAxes < 3) {
      ...
      return function () {
        var halfLength = canvasSize / 2;
        var origin = { x: halfLength, y: halfLength };

        function chart(data) {
          var canvas = createCanvas(canvasSize);
        ...
        function drawPath(ctx, origin, radii) {
          ...
          for (var i = 1; i < radii.length; i++) {
            ctx.lineTo(radii[i] * Math.cos(angleIncrement * i), radii[i] * Math.sin(angleIncrement * i));
          }
          ctx.closePath();
        ...
        function drawAxes(ctx, origin, radius, numSides) {
          ...
          for (var i = 0; i < numSides; i++) {
            ...
            var x = radius * Math.cos(a * i);
            var y = radius * Math.sin(a * i);
            ctx.lineTo(x, y);
            ctx.stroke();
          }
